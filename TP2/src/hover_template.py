...
def get_hover_template(name, mode):
    ...
    '''
    title = f'<span style="font-family: Grenze Gotisch; font-size: 24px; color: black;">{name}</span>'

    if mode == MODES['count']:
        amount = '{y}'
        player_line = '%{format} lines'.format(format=amount)
    else:
        amount = '{y:.2f}%'
        player_line = '%{format} of lines'.format(format=amount)
        
    return title + '<br>' + player_line + '<extra></extra>'